...
class Config:
    ...
    OU_ENTRY_Z = 1.8             # порог z-score для входа (абсолютное значение)
    OU_EXIT_Z = 0.3              # порог z-score для выхода (возврат к среднему)
    OU_STOP_Z = 3.5              # порог z-score для принудительного стоп-лосса
    OU_DELTA_T = 1.0             # интервал времени между свечами (обычно 1)

    # Параметры демо-трейдера
    DEMO_INITIAL_BALANCE = 10000.0    # стартовый баланс в USDT
    ...
    DEMO_ENTRY_Z = 1.8                # порог z-score для входа
